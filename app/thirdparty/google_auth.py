...
def saveIntoDatabase(gaccountinfo, credentials):
    ...
    try:
        user = CloudCV_Users(first_name=user_fname, last_name=user_lname, userid=user_uuid,emailid=user_emailid,is_active=True)
        user.save()
...
def handleCallback(code, request):
    ...
    result = requests.get('https://www.googleapis.com/plus/v1/people/me', headers={'Authorization': 'Bearer '+str(cred_json['access_token'])})
