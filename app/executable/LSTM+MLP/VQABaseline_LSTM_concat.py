...
class Config:
	def __init__(self):
		...
		self.df_path = '/srv/share/data/visualAttention_oct17.pkl'
...
class VQABaseline_LSTM:
	...
	def loadModel(self, model_path=None, weights=None):
		#if model_path is None:
		#	self.model = pickle.load(file('/home/abhshkdz/vqa-attention-master/vqa-attention/cv/lr_0.001_weights_1024_1000_fc7.hdf5.pkl', 'rb'))
		#else:
		#	self.model = pickle.load(file(model_path, 'rb'))
		left = Sequential()
		left.add(Embedding(12496, 200));
		left.add(LSTM(output_dim=1024, return_sequences=False))

		right = Sequential()
		right.add(Dense(1024, input_dim=4096))
		right.add(Activation('tanh'))
